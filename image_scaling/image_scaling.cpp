...
struct ImgReSampleContext {
    int      width_src, height_src, width_dst, height_dst;
    int      h_incr, v_incr;
...
#if NB_TAPS == 4
        sum = s[ 0 ] * filter[ 0 ] + s[ 1 ] * filter[ 1 ] + s[ 2 ] * filter[ 2 ] + s[ 3 ] * filter[ 3 ];
#else
        {
            int j;
            sum = 0;
...
/**
 * @brief slow version to handle limit cases. Does not need optimisation
 *
 * @param dst
 * @param dst_width
 * @param src
 ...
 * @param height_src
 */
static void component_resample( ImgReSampleContext* s, uint8_t* output, int owrap, int width_dst, int height_dst, uint8_t* input, int iwrap, int width_src, int height_src ) {
    ...
    for ( y = 0; y < height_dst; y++ ) {
        ...
        while ( last_src_y < src_y1 ) {
            ...
            else if ( y1 >= height_src ) {
                y1 = height_src - 1;
            }
            src_line = input + y1 * iwrap;
            new_line = s->line_buf + ring_y * width_dst;
            /* apply filter and handle limit cases correctly */
            h_resample( new_line, width_dst, src_line, width_src, -FCENTER * POS_FRAC, s->h_incr, &s->h_filters[ 0 ][ 0 ] );
            /* handle ring buffer wraping */
            if ( ring_y >= LINE_BUF_HEIGHT ) {
                memcpy( s->line_buf + ( ring_y - LINE_BUF_HEIGHT ) * width_dst, new_line, width_dst );
            }
        }
        /* apply vertical filter */
        phase_y = get_phase( src_y );
        v_resample( output, width_dst, s->line_buf + ( ring_y - NB_TAPS + 1 ) * width_dst, width_dst, &s->v_filters[ phase_y ][ 0 ] );
        src_y += s->v_incr;
        output += owrap;
    }
}
 ...
 *
 * @param filter
 * @param factor 缩放比率=输出/输入
 * @param tap_count 阶数
 * @param phase_count 相数
 * @param scale
 * @param type
 */
 ...
 * @brief
 *
 * @param width_dst output_width
 * @param height_dst output_height
 * @param width_src input_width
 * @param height_src input_height
 * @return ImgReSampleContext*
 */
ImgReSampleContext* img_resample_full_init( int width_dst, int height_dst, int width_src, int height_src ) {
    ...
    // (LINE_BUF_HEIGHT + NB_TAPS) 是干啥的？
    if ( ( unsigned )width_dst >= UINT_MAX / ( LINE_BUF_HEIGHT + NB_TAPS ) )
        return NULL;
    s->line_buf = ( uint8_t* )av_mallocz( width_dst * ( LINE_BUF_HEIGHT + NB_TAPS ) );
    if ( !s->line_buf ) {
        av_free( s );
        return NULL;
    }

    s->width_dst  = width_dst;
    s->height_dst = height_dst;
    s->width_src  = width_src;
    s->height_src = height_src;

    s->h_incr = ( width_src * POS_FRAC ) / width_dst;
    s->v_incr = ( height_src * POS_FRAC ) / height_dst;

    /** last argument should be 0, 1 or 2 ***/
    build_filter( &s->h_filters[ 0 ][ 0 ], ( float )width_dst / ( float )( width_src ), NB_TAPS, NB_PHASES, 1 << FILTER_BITS, 0 );
    build_filter( &s->v_filters[ 0 ][ 0 ], ( float )height_dst / ( float )( height_src ), NB_TAPS, NB_PHASES, 1 << FILTER_BITS, 0 );

    return s;
}

ImgReSampleContext* img_resample_init( int width_dst, int height_dst, int width_src, int height_src ) {
    return img_resample_full_init( width_dst, height_dst, width_src, height_src );
}
...
void img_resample( ImgReSampleContext* s, MyPicture* output, const MyPicture* input ) {
    ...
    for ( i = 0; i < 3; i++ ) {
        ...
        component_resample( s, optr, output->linesize[ i ], s->width_dst >> shift, s->height_dst >> shift, input->data[ i ], input->linesize[ i ], s->width_src >> shift, s->height_src >> shift );
    }
}
 ...
 * @brief
 *
 * @param columns_src input_width
 * @param rows_src input_height
 * @param columns_dst output_width
 * @param rows_dst output_height
 * @param ft 输出文件
 */
void polyphase( int columns_src, int rows_src, int columns_dst, int rows_dst, FILE* ft ) {
    ImgReSampleContext* s;

    /***** for y ****/
    s = img_resample_full_init( columns_dst, rows_dst, columns_src, rows_src );
    // dump_filter(&s->h_filters[0][0]);
    component_resample( s, img2, s->width_dst, s->width_dst, s->height_dst, img, s->width_src, s->width_src, s->height_src );
    img_resample_close( s );
    fwrite( img2, 1, columns_dst * rows_dst, ft );
    memcpy( img1, img2, columns_dst * rows_dst );

    /**** for u ****/
    s = img_resample_full_init( columns_dst / 2, rows_dst / 2, columns_src / 2, rows_src / 2 );
    component_resample( s, img2, columns_dst / 2, columns_dst / 2, rows_dst / 2, img + ( columns_src * rows_src ), columns_src / 2, columns_src / 2, rows_src / 2 );
    img_resample_close( s );
    fwrite( img2, 1, ( columns_dst / 2 ) * ( rows_dst / 2 ), ft );
    memcpy( img1 + ( columns_dst * rows_dst ), img2, ( columns_dst * rows_dst ) / 4 );

    /**** for v ****/
    s = img_resample_full_init( columns_dst / 2, rows_dst / 2, columns_src / 2, rows_src / 2 );
    component_resample( s, img2, columns_dst / 2, columns_dst / 2, rows_dst / 2, img + ( int )( columns_src * rows_src * 1.25 ), columns_src / 2, columns_src / 2, rows_src / 2 );
    img_resample_close( s );
    fwrite( img2, 1, ( columns_dst / 2 ) * ( rows_dst / 2 ), ft );
    memcpy( img1 + ( int )( columns_dst * rows_dst * 1.25 ), img2, ( columns_dst * rows_dst ) / 4 );
...
void bilinear( int input_width, int input_height, int output_width, int output_height, FILE* ft ) {
    ...
    for ( i = 0; i < output_height; ++i ) {
        for ( j = 0; j < output_width; ++j ) {
            ...
            ceil_x  = floor_x + 1;
            if ( ceil_x >= input_width )
                ceil_x = floor_x;
            ceil_y = floor_y + 1;
            if ( ceil_y >= input_height )
                ceil_y = floor_y;
    ...
    for ( i = 0; i < output_height / 2; ++i ) {
        for ( j = 0; j < output_width / 2; ++j ) {
            ...
            floor_y = ( int )( i * nYFactor );
            ceil_x  = floor_x + 1;
            if ( ceil_x >= input_width / 2 )
                ceil_x = floor_x;
            ceil_y = floor_y + 1;
            if ( ceil_y >= input_height / 2 )
                ceil_y = floor_y;
            fraction_x  = j * nXFactor - floor_x;
            fraction_y  = i * nYFactor - floor_y;
            one_minus_x = 1.0 - fraction_x;
